...
struct PackedVirtInfo f_virt_info_generate() {
  ...
  while (true) {
    ...
    virt_info.virt_ip = inet_addr(("192.168." + std::to_string(third_byte) +
                                   "." + std::to_string(fourth_byte)).c_str());
    ...
    if (used_virt_info.find({used_ip, used_port}) == 
        used_virt_info.end()) {
...
void f_pkt_construct(struct rte_mbuf* mbuf, 
  ...
  rte_memcpy(eth_hdr->dst_addr.addr_bytes,
             &neighbor_mac[0], RTE_ETHER_ADDR_LEN);
...
void f_virt_info_req_manage(struct rte_mbuf *recv_req, 
  ...
  ip_hdr = rte_pktmbuf_mtod_offset(recv_req, struct rte_ipv4_hdr *, 
                                   hdr_offset);
  hdr_offset += sizeof(struct rte_ipv4_hdr);
  udp_hdr = rte_pktmbuf_mtod_offset(recv_req, struct rte_udp_hdr *, 
                                    hdr_offset);
  hdr_offset += sizeof(struct rte_udp_hdr);
  req = rte_pktmbuf_mtod_offset(recv_req, struct PackedVirtInfoReq *,
                                hdr_offset);
  struct PackedVirtInfo virt_info = f_virt_info_generate();
  struct PackedVirtInfoKey virt_info_key = {virt_info.virt_ip, 
                                            virt_info.virt_port,
                                            req->protocol};
  ...
  int status = rte_pktmbuf_alloc_bulk(queue_config->mempool_ptr, 
                                      mbufs_send, pkt_send_num);
  if (status != 0)
  ...
  for (uint32_t i = 0; i < pkt_send_num; i++) {
    ...
    f_pkt_construct(mbufs_send[i], g_mac_table[g_lb_ips[i]], 
                    buffer, sizeof(buffer), slb_endpoint);
  }
  rte_eth_tx_burst(g_nic_info_ctrl_plane.id, queue_config->queue_id, 
                   mbufs_send, pkt_send_num);
}
...
void f_lb_reply_manage(struct rte_mbuf *recv_reply,
  ...
  if (req_label_reply_count[req_label] == g_lb_ips.size()) {
    ...
    f_pkt_construct(mbuf_send, g_mac_table[endpoint.ip], &virt_info_reply, 
                    sizeof(struct PackedVirtInfo), endpoint);
    rte_eth_tx_burst(g_nic_info_ctrl_plane.id, 
                     queue_config->queue_id, &mbuf_send, 1);
    req_label_reply_count[req_label] = 0;
