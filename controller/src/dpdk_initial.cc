...
f_dpdk_nic_queue_initial(struct rte_mempool* mempool_ptr, int dpdk_nic_id)
{
    ...
    for (int i = 0; i < queue_num; i++) {
        if (rte_eth_rx_queue_setup(dpdk_nic_id, i, 1024, rte_eth_dev_socket_id(dpdk_nic_id), NULL, mempool_ptr) < 0) {
            rte_exit(EXIT_FAILURE, "Could not setup RX queue %d.\n", i);
        }
        struct rte_eth_txconf tx_conf = dev_info.default_txconf;
        tx_conf.offloads = offloads;
        if (rte_eth_tx_queue_setup(dpdk_nic_id, i, 1024, rte_eth_dev_socket_id(dpdk_nic_id), &tx_conf) < 0) {
    ...
    if (!rte_flow_create(dpdk_nic_id, &attr, pattern, action, &error)) {
        rte_exit(
            EXIT_FAILURE,
            "Error initialize udp for queue 0"
            " at f_dpdk_nic_queue_initial.");
    }
    ...
    if (!rte_flow_create(dpdk_nic_id, &attr, pattern, action, &error)) {
        rte_exit(
            EXIT_FAILURE,
            "Error initialize udp for queue 1"
            " at f_dpdk_nic_queue_initial.");
    }
    ...
    if (!rte_flow_create(dpdk_nic_id, &attr, pattern, action, &error)) {
        rte_exit(
            EXIT_FAILURE,
            "Error initialize udp for queue 2"
            " at f_dpdk_nic_queue_initial.");
    }
}