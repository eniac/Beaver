...
f_ss_new_tail_wait(uint32_t ssid, uint32_t max_flight_ss_num)
{
    ...
    ss_tail_cv.wait(lock, [ssid, max_flight_ss_num] { return ssid - ss_tail < max_flight_ss_num; });
}
...
f_arp_mac_req_send(struct QueueConfig* queue_config, uint32_t dst_ip)
{
    ...
    rte_memcpy(eth_hdr->dst_addr.addr_bytes, &kBroadcastMac[0], RTE_ETHER_ADDR_LEN);
    ...
    rte_memcpy(arp_hdr->arp_data.arp_tha.addr_bytes, &kBroadcastMac[0], RTE_ETHER_ADDR_LEN);
    arp_hdr->arp_data.arp_sip = htonl(g_nic_info_ctrl_plane.ip);
    arp_hdr->arp_data.arp_tip = htonl(dst_ip);

    uint32_t pkt_sent_count = rte_eth_tx_burst(g_nic_info_ctrl_plane.id, queue_config->queue_id, &mbuf_send, 1);
    if (pkt_sent_count < 1)
...
f_ss_tail_pkts_recv(__attribute__((unused)) void* arg)
{
    ...
    while (true) {
        recv_pkt_num = rte_eth_rx_burst(g_nic_info_ctrl_plane.id, queue_config->queue_id, mbufs_recv, kRecvBurstSize);
        for (uint32_t i = 0; i < recv_pkt_num; i++) {
            ...
            recv_ss_tail = ntohl(*rte_pktmbuf_mtod_offset(mbufs_recv[i], uint32_t*, kUdpHdrSize));
            auto iter = std::find(g_backend_ips.begin(), g_backend_ips.end(), src_ip);
...
f_ss_pkts_send(__attribute__((unused)) void* arg)
{
    ...
    while (ssid < g_max_ss_id) {
        ...
        if (cur_time - last_time < ss_interval)
            ...
            last_time = cur_time;
        if (rte_pktmbuf_alloc_bulk(queue_config->mempool_ptr, mbufs_send, g_lb_ips.size()) != 0) {
            rte_exit(
                EXIT_FAILURE,
                "Error when rte_pktmbuf_alloc_bulk"
                " at f_ss_pkts_sent\n");
        }
        ...
        for (uint32_t i = 0; i < g_lb_ips.size(); i++) {
            ...
            f_pkt_construct(mbufs_send[i], g_mac_table[g_lb_ips[i]], data, 5, endpoint);
        }
        first_sent_time[ssid] = f_clock_time_get();
        pkt_sent_count =
            rte_eth_tx_burst(g_nic_info_ctrl_plane.id, queue_config->queue_id, mbufs_send, g_lb_ips.size());
        ...
        for (uint32_t i = 0; i < pkt_recv_num; i++) {
            ...
            if (eth_hdr->ether_type == htons(RTE_ETHER_TYPE_ARP)) {
                struct rte_arp_hdr* arp_hdr =
                    rte_pktmbuf_mtod_offset(mbufs_recv[i], struct rte_arp_hdr*, sizeof(struct rte_ether_hdr));
                if (arp_hdr->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REPLY)) {
                    ...
                    neighbor_mac = std::vector<uint8_t>(
                        arp_hdr->arp_data.arp_sha.addr_bytes, arp_hdr->arp_data.arp_sha.addr_bytes + 6);
                ...
                struct rte_udp_hdr* udp_hdr = rte_pktmbuf_mtod_offset(
                    mbufs_recv[i], struct rte_udp_hdr*, sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr));
