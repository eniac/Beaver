...
f_initial_data_get(const std::string& file_path)
{
    ...
    std::unordered_map<std::string, uint8_t> protocol_map = {{"tcp", 6}, {"udp", 17}};
    ...
    for (const auto& virt_info_entry : json_data["services"]) {
        ...
        three_tuple_key.ip = ntohl(inet_addr(virt_info_entry["virt_ip"].asCString()));
        auto port_str = virt_info_entry["virt_port"].asString();
        three_tuple_key.port = static_cast<uint16_t>(std::stoi(port_str));
        three_tuple_key.protocol = protocol_map[virt_info_entry["protocol"].asString()];
        ...
        if (three_tuple_map.find(three_tuple_key) != three_tuple_map.end()) {
            ...
            endpoint_list.insert(endpoint_list.end(), endpoints.begin(), endpoints.end());
        } else
