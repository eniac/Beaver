...
f_nic_info_get(const std::string& ip_prefix)
{
    ...
    for (curr_iface = iface_addrs; curr_iface != NULL; curr_iface = curr_iface->ifa_next) {
        if (curr_iface->ifa_addr == NULL)
            continue;
        addr_family = curr_iface->ifa_addr->sa_family;
        if (addr_family == AF_INET || addr_family == AF_INET6) {
            uint32_t sock_size = (addr_family == AF_INET) ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6);
            status = getnameinfo(curr_iface->ifa_addr, sock_size, host_addr, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
            if (status != 0) {
...
f_nic_info_get_data_plane(const std::string& ip_prefix)
{
    ...
    for (curr_iface = iface_addrs; curr_iface != NULL; curr_iface = curr_iface->ifa_next) {
        ...
        if (addr_family == AF_INET || addr_family == AF_INET6) {
            ...
            std::string ip_addr(host_addr);
            if (ip_addr.find(ip_prefix) == 0 && ip_addr.find("192.168.255") != 0) {
                DpdkNicInfo nic_info;
                nic_info.name = curr_iface->ifa_name;
                nic_info.ip = ntohl(inet_addr(ip_addr.c_str()));
                nic_info.mac = f_nic_mac_get(nic_info.name);
