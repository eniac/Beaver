...
int f_load_balance(__attribute__((unused)) void *arg) {
  ...
  while (true) {
    pkt_recv_num = rte_eth_rx_burst(g_nic_info_data_plane.id,
                                    queue_config->queue_id, 
                                    mbufs_recv, kRecvBurstSize);
    int alloc_success = rte_pktmbuf_alloc_bulk(queue_config->mempool_ptr, 
                                                mbufs_send, pkt_recv_num);
	if (alloc_success != 0)
    ...
    while (pkt_handled_count < pkt_recv_num) {
      pkt_has_sent = f_load_balance_pkt(mbufs_recv[pkt_handled_count],
                                        mbufs_send[pkt_sent_count], 
                                        queue_config->queue_id);
      if (pkt_has_sent) {
        pkt_sent_count++;
        pkt_has_sent = false;
    ...
    if (pkt_sent_count > 0) {
      pkt_sent_num = rte_eth_tx_burst(g_nic_info_data_plane.id, 
                                      queue_config->queue_id, 
                                      mbufs_send, pkt_sent_count);
...
bool f_load_balance_pkt(struct rte_mbuf *recv_pkt, 
  ...
  ip_hdr = rte_pktmbuf_mtod_offset(recv_pkt, struct rte_ipv4_hdr *,
                                   hdr_offset);
  ...
  udp_hdr = rte_pktmbuf_mtod_offset(recv_pkt, struct rte_udp_hdr *, 
                                    hdr_offset);
  PackedFiveTupleKey five_tuple_key(ntohl(ip_hdr->src_addr),
                                    ntohl(ip_hdr->dst_addr), 
                                    ip_hdr->next_proto_id, 
                                    ntohs(udp_hdr->src_port), 
                                    ntohs(udp_hdr->dst_port));
  struct PackedEndpoint endpoint = f_endpoint_hash(
      g_map_five_tuple_endpoint[queue_id], five_tuple_key);
    ...
    pkt_ssid = ip_hdr->type_of_service + (ip_hdr->time_to_live >> 2) * 256;
    {
      ...
      if (ssid_remainder <= 8192) {
        if (pkt_ssid > ssid_remainder && 
            pkt_ssid < ssid_remainder + 8191) {
          uint32_t new_ssid = pkt_ssid + 16384 * (ssid / 16384);
          ssid = new_ssid;
        }
      }
      else {
        if (!(pkt_ssid > ssid_remainder - 8192 && 
              pkt_ssid <= ssid_remainder)) {
          uint32_t new_ssid = pkt_ssid + 16384 * (ssid / 16384);
					if (pkt_ssid < ssid_remainder)
						new_ssid += 16384;
          ssid = new_ssid;
  ...
  if (!endpoint.is_zero()) {
    f_pkt_construct_data_plane(mbuf_send, neighbor_mac, 
                               (void*) eth_hdr, 
                               ntohs(ip_hdr->total_length) + 14, 
                               endpoint);
    return true;
  }
  return false;
...
int f_ctrl_pkts_update(__attribute__((unused)) void *arg) {
  ...
  while (true) {
    ...
    pkt_sent_count = 0;
    while (pkt_handled_count < pkt_recv_num) {
      pkt_has_sent = f_ctrl_pkt_handle(mbufs_recv[pkt_handled_count],
                                       mbufs_send[pkt_sent_count]);
      if (pkt_has_sent) {
        pkt_sent_count++;
        pkt_has_sent = false;
    ...
    if (pkt_sent_count > 0) {
      pkt_sent_num = rte_eth_tx_burst(g_nic_info_ctrl_plane.id, 
                                      queue_config->queue_id, 
                                      mbufs_send, pkt_sent_count);
      if (pkt_sent_num < pkt_sent_count) {
        for (uint32_t i = pkt_sent_num; i < pkt_recv_num; i++)
...
int f_ctrl_pkt_handle(struct rte_mbuf *recv_pkt, struct rte_mbuf *mbuf_send) {
  ...
  if (eth_hdr->ether_type == htons(RTE_ETHER_TYPE_ARP)) {
    struct rte_arp_hdr *arp_hdr = rte_pktmbuf_mtod_offset(
        recv_pkt, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
  ...
  struct rte_udp_hdr *udp_hdr = rte_pktmbuf_mtod_offset(
      recv_pkt, struct rte_udp_hdr *, hdr_offset);
  ...
  if (*request_type == 1) {
    ...
    struct PackedEndpoint controller(g_controller_ip, 
                                     ntohs(udp_hdr->src_port));
    f_pkt_construct_ctrl_plane(mbuf_send, neighbor_mac, 
                               (void*) &label, sizeof(label), controller);
    return true;
  }
  return false;
}
...
void f_pkt_construct_data_plane(struct rte_mbuf* mbuf, 
  ...
  rte_memcpy(eth_hdr->src_addr.addr_bytes, 
             g_nic_info_data_plane.mac.data(), RTE_ETHER_ADDR_LEN);
  rte_memcpy(eth_hdr->dst_addr.addr_bytes,
             neighbor_mac.data(), RTE_ETHER_ADDR_LEN);
...
void f_pkt_construct_ctrl_plane(struct rte_mbuf* mbuf, 
  ...
  struct rte_ether_hdr *eth_hdr = (struct rte_ether_hdr *) pkt_data;
  rte_memcpy(eth_hdr->src_addr.addr_bytes, 
             g_nic_info_ctrl_plane.mac.data(), RTE_ETHER_ADDR_LEN);
  rte_memcpy(eth_hdr->dst_addr.addr_bytes,
             neighbor_mac.data(), RTE_ETHER_ADDR_LEN);
  eth_hdr->ether_type = htons(RTE_ETHER_TYPE_IPV4);
