...
def f_config_send_single_node(ssh_ip, user_name, key_path, 
...
def f_compile_single_node(ssh_ip, user_name, key_path, 
                          node_type):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
...
def f_run_prog_single_node(ssh_ip, user_name, key_path, 
                           node_type):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
  if (node_type == 'backend'):
    run_cmds = ['cd backend && make load', 
                f'cd {node_type} && nohup sudo stdbuf -o0 ./main user '
                '> /dev/null 2>&1 &']
  elif (node_type == 'controller'):
    run_cmds = [f'cd {node_type} && nohup sudo stdbuf -o0 ./main > controller.log 2>&1 &']
  else:
    run_cmds = [f'cd {node_type} && nohup sudo stdbuf -o0 ./main > /dev/null 2>&1 &']
  f_cmds_exec(ssh_connection, run_cmds)
  ssh_connection.close()


def f_latency_prog_single_node(ssh_ip, user_name, key_path, 
                               node_type, latency_type):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
  if (node_type == 'backend'):
    run_cmds = ['cd backend && make load', 
    ...
    run_cmds = [f'cd {node_type} && nohup sudo stdbuf -o0 ./main '
                f'{latency_type} > /dev/null 2>&1 &']
  elif (node_type == 'load_balancer'):
    run_cmds = [f'cd {node_type} && nohup sudo stdbuf -o0 ./main 1 > '
                '/dev/null 2>&1 &']
  f_cmds_exec(ssh_connection, run_cmds)
  ssh_connection.close()
...
def f_bot_prog_single_node(ssh_ip, user_name, key_path, node_type):
  ...
  if (node_type == 'backend'):
    run_cmds = ['cd backend && make load', 
                f'cd {node_type} && nohup sudo stdbuf -o0 ./main user '
                '> /dev/null 2>&1 &',
                f'cd {node_type} && nohup sudo stdbuf -o0 ./main bot '
                '> /dev/null 2>&1 &']
  elif (node_type == 'controller'):
    run_cmds = [f'cd {node_type} && nohup sudo stdbuf -o0 ./main > controller.log 2>&1 &']
  else:
...
def f_bot_laiyang_prog_single_node(ssh_ip, user_name, key_path, 
                                   node_type, backends_index, real_index):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
  index = -1;
  if (node_type == 'backend'):
...
def f_remove_prog_single_node(ssh_ip, user_name, key_path, 
                              node_type):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
  cmds = ["ps aux | grep ./main | grep -v grep | awk '{{print $2}}' | xargs sudo kill -9"]
  if (node_type == 'backend'):
...
def f_load_iperf_request_single_node(ssh_ip, user_name, key_path, 
                                     request, thread_num):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
...
def f_remove_rate_service_single_node(ssh_ip, user_name, key_path):
  ssh_connection = f_ssh_connection_create(ssh_ip, user_name, key_path)
  cmds = ["ps aux | grep iperf | grep -v grep | awk '{{print $2}}' | xargs sudo kill -9"]
  f_cmds_exec(ssh_connection, cmds)
  ssh_connection.close()
...
def f_cassandra_env_config_single_node(ssh_ip, user_name, key_path, node_type):
  ...
  cmds = []
  if (node_type == 'backend'):
    cmds.append("echo \"deb https://debian.cassandra.apache.org 41x main\" "
                "| sudo tee -a /etc/apt/sources.list.d/cassandra.sources.list")
    cmds.append("curl https://downloads.apache.org/cassandra/KEYS "
                "| sudo apt-key add -")
    cmds.append("sudo apt-get update -y")
    cmds.append("sudo apt-get install cassandra cassandra-tools -y")
  elif (node_type == 'client'):
    cmds.append("curl -O --location https://github.com/brianfrankcooper"
...
def f_rate_ss_deploy(xml_file_path, user_name, key_path, 
  ...
  cmds = []
  if (if_parallel):
    cmds.append(f'cd controller && nohup sudo stdbuf -o0 '
...
def f_bound_ss_deploy(xml_file_path, user_name, key_path, 
                         nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  controller_index = nodes_config['controller'][0]
  controller_name = 'node' + str(controller_index)
...
def f_accuracy_ss_deploy(xml_file_path, user_name, key_path, 
...
def f_load_ss_deploy(xml_file_path, user_name, key_path, 
                     nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  controller_index = nodes_config['controller'][0]
  controller_name = 'node' + str(controller_index)
...
def f_bot_ss_deploy(xml_file_path, user_name, key_path, 
                    nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  controller_index = nodes_config['controller'][0]
  controller_name = 'node' + str(controller_index)
...
def f_rate_ss_result_pull(xml_file_path, user_name, key_path, 
...
def f_bound_ss_result_pull(xml_file_path, user_name, key_path, 
                          nodes_config, local_file_path):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  controller_index = nodes_config['controller'][0]
  controller_name = 'node' + str(controller_index)
...
def f_accuracy_ss_result_pull(xml_file_path, user_name, key_path, 
                          nodes_config, local_file_path):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  controller_index = nodes_config['controller'][0]
  controller_name = 'node' + str(controller_index)
...
def f_latency_intra_result_pull(xml_file_path, user_name, key_path, 
                                nodes_config, local_file_path):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
...
def f_bot_result_pull(xml_file_path, user_name, key_path, 
                      nodes_config, local_file_path):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
...
def f_load_cassandra_result_pull(xml_file_path, user_name, key_path, 
                          nodes_config, local_file_path):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  client_index = nodes_config['client'][0]
  client_name = 'node' + str(client_index)
...
def f_load_iperf_result_pull(xml_file_path, user_name, key_path, 
