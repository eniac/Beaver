...
int main(int argc, char *argv[]) {
  ...
  struct rte_mempool *mempool_ptr_ctrl_plane = rte_pktmbuf_pool_create(
      "mempool_ptr_ctrl", kMbufsNum, kMbufsCacheSize, 0, 
      RTE_MBUF_DEFAULT_BUF_SIZE, rte_socket_id());
  f_dpdk_nic_queue_initial(mempool_ptr_ctrl_plane, g_nic_info_ctrl_plane.id);

  struct QueueConfig *queue_config = 
      new QueueConfig(mempool_ptr_ctrl_plane, 0);

  if (rte_eal_remote_launch(f_ss_pkts_recv, queue_config, 2) < 0)
    rte_exit(EXIT_FAILURE, "Error launching thread on lcore %d\n", 2);

  struct QueueConfig *queue_config_virt_req = 
      new QueueConfig(mempool_ptr_ctrl_plane, 1);
  if (rte_eal_remote_launch(f_virt_info_reqs_manage, 
                            queue_config_virt_req, 3) < 0) {
    rte_exit(EXIT_FAILURE, "Error launching thread on lcore %d\n", 3);
  }

  struct QueueConfig *queue_config_ss_tail = 
      new QueueConfig(mempool_ptr_ctrl_plane, 2);
  if (rte_eal_remote_launch(f_ss_tail_pkts_recv, 
                            queue_config_ss_tail, 4) < 0) {
    rte_exit(EXIT_FAILURE, "Error launching thread on lcore %d\n", 4);
  }
  struct Data {
    int frequency;
