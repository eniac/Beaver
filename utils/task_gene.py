...
def f_set_env(xml_file_path, user_name, key_path, env_install_file_path):
  ...
  for node_name, ssh_ip in ssh_ips.items():
    thread = threading.Thread(target = f_set_env_single_node, 
                              args = (ssh_ip, user_name, 
                                      key_path, env_install_file_path))
    threads.append(thread)
  for thread in threads:
    thread.start()
  for thread in threads:
...
def f_folders_send(xml_file_path, user_name, key_path, 
  ...
  for node_type, nodes_index in nodes_config.items():
    for index in nodes_index:
      ...
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_folder_send_single_node, 
                                args = (ssh_ip, user_name, 
                                        key_path, base_path, node_type))
      threads.append(thread)
  if ('external' in ssh_ips.keys()):
    ssh_ip = ssh_ips['external']
    thread = threading.Thread(target = f_folder_send_single_node, 
                              args = (ssh_ip, user_name, 
                                      key_path, base_path, 'client'))
    threads.append(thread)
  for thread in threads:
    thread.start()
  for thread in threads:
...
def f_delete_folders(xml_file_path, user_name, key_path,
                     nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  ...
  for node_type, nodes_index in nodes_config.items():
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_folder_delete_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type))
      threads.append(thread)
  if ('external' in ssh_ips.keys()):
    ssh_ip = ssh_ips['external']
...
def f_replace_folders(xml_file_path, user_name, key_path,
                      config_file_path, folder_type):
  try:
  ...
  for index in nodes_index:
    node_name = 'node' + str(index)
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_folder_files_delete_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      folder_type))
    threads.append(thread)
  for thread in threads:
  ...
  for index in nodes_index:
    node_name = 'node' + str(index)
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_config_send_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      config_file_path, remote_config_path))
    threads.append(thread)
    thread = threading.Thread(target = f_compile_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      folder_type))
    threads.append(thread)
  for thread in threads:
...
def f_config_send(xml_file_path, user_name, key_path, 
                  base_path, nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
...
def f_compile(xml_file_path, user_name, key_path, 
              nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
...
def f_rate_progs_run(xml_file_path, user_name, key_path, 
                     nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  controller_threads = []
  ...
  for node_type, nodes_index in nodes_config.items():
    ...
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_run_prog_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type))
...
def f_latency_intra_progs_run(xml_file_path, user_name, key_path, 
                              nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  client_threads = []
  ...
  for node_type, nodes_index in nodes_config.items():
    ...
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_latency_prog_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type, 'intra'))
      if (node_type == 'client'):
...
def f_latency_internet_progs_run(xml_file_path, user_name, key_path, 
                              nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  client_threads = []
...
def f_bot_programs_run(xml_file_path, user_name, key_path, 
                       nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  print("Start running programs.")
  for node_type, nodes_index in nodes_config.items():
    ...
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_bot_prog_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type))
      threads.append(thread)
  for thread in threads:
...
def f_bot_poll_programs_run(xml_file_path, user_name, key_path, 
                            nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  print("Start running programs.")
  for node_type, nodes_index in nodes_config.items():
    ...
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_bot_poll_prog_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type))
      threads.append(thread)
  for thread in threads:
...
def f_bot_laiyang_programs_run(xml_file_path, user_name, key_path, 
                               nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  ...
  for node_type, nodes_index in nodes_config.items():
    ...
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_bot_laiyang_prog_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type, backends_index, index))
      threads.append(thread)
  for thread in threads:
    thread.start()
  for thread in threads:
...
def f_rate_progs_remove(xml_file_path, user_name, key_path, 
                        nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  ...
  for node_type, nodes_index in nodes_config.items():
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_remove_prog_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type))
      threads.append(thread)
  for thread in threads:
...
def f_latency_intra_progs_remove(xml_file_path, user_name, key_path, 
                                 nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  # print("Start removing programs.")
...
def f_rate_services_deploy(xml_file_path, user_name, key_path, 
                           services):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  ...
  for service in services:
    ...
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_run_rate_service_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      service))
    threads.append(thread)
  for thread in threads:
...
def f_load_cassandra_deploy(xml_file_path, user_name, key_path, services):
  ...
  for service in services:
    node_name = service['node_name']
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_run_cassandra_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      service))
    threads.append(thread)
  for thread in threads:
...
def f_load_cassandra_intial(xml_file_path, user_name, key_path, backend_index):
  ...
  cmds = [f'cqlsh 192.168.{backend_index}.2 9042 -e "CREATE KEYSPACE ycsb WITH REPLICATION = '
...
def f_rate_requests_deploy(xml_file_path, user_name, key_path, 
                           requests):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  ...
  for request in requests:
    ...
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_run_rate_request_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      request))
    threads.append(thread)
  for thread in threads:
...
def f_load_iperf_requests_deploy(xml_file_path, user_name, key_path, 
                                 requests, thread_num):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  print("Start launching requests.")
  for request in requests:
    node_name = request['node_name']
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_load_iperf_request_single_node, 
                              args = (ssh_ip, user_name, key_path, 
                                      request, thread_num))
    threads.append(thread)
  for thread in threads:
...
def f_load_cassandra_request(xml_file_path, user_name, key_path, 
                             requests, operation):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  request = requests[0]
  node_name = request['node_name']
  ...
  cmds = [f'cd ycsb && python2 ./bin/ycsb load cassandra-cql '
          f'-p hosts={virt_ip} -p port={virt_port} -P workloads/{workload} '
          f'-p operationcount={operation_count} -threads 1']
  cmds.append(f'cd ycsb && python2 ./bin/ycsb run cassandra-cql '
              f'-p hosts={virt_ip} -p port={virt_port} -P workloads/{workload} '
...
def f_rate_services_remove(xml_file_path, user_name, key_path, nodes_config):
  ...
  for index in backends_index:
    node_name = 'node' + str(index)
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_remove_rate_service_single_node, 
                              args = (ssh_ip, user_name, key_path))
    threads.append(thread)
...
def f_load_cassandra_env_config(xml_file_path, user_name, key_path, 
                                nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  ...
  for node_type, nodes_index in nodes_config.items():
    ...
    for index in nodes_index:
      node_name = 'node' + str(index)
      ssh_ip = ssh_ips[node_name]
      thread = threading.Thread(target = f_cassandra_env_config_single_node, 
                                args = (ssh_ip, user_name, key_path, 
                                        node_type))
      threads.append(thread)
  for thread in threads:
...
def f_load_cassandra_remove(xml_file_path, user_name, key_path, 
                                     nodes_config):
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  threads = []
  backends_index = nodes_config['backend']
  ...
  for index in backends_index:
    node_name = 'node' + str(index)
    ssh_ip = ssh_ips[node_name]
    thread = threading.Thread(target = f_remove_cassandra_single_node, 
                              args = (ssh_ip, user_name, key_path))
    threads.append(thread)
  for thread in threads:
    thread.start()
