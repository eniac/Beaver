...
def f_rate_config(xml_file_path, user_name, key_path, 
                  lb_num, base_path):
  ...
  f_folders_send(xml_file_path, user_name, key_path,
                 base_path, nodes_config)
  f_config_send(xml_file_path, user_name, key_path,
                base_path, nodes_config)
  f_compile(xml_file_path, user_name, key_path, nodes_config)
  f_sw_config_gene(xml_file_path, nodes_config['load_balancer'])
  print('Have finsih all the configurations,'
        'please use the output commands to configure the switch.')


def f_load_config(xml_file_path, user_name, key_path,
                  lb_num, load_type, base_path):
  config_file_path = os.path.join(base_path, 'config.json')
  active_nodes = f_active_nodes_get(xml_file_path, user_name, key_path)
  nodes_config = f_load_config_gene(lb_num, load_type, config_file_path,
                                    active_nodes)
  if ('cassandra' in load_type):
    f_load_cassandra_env_config(xml_file_path, user_name, key_path, 
                                nodes_config)
  f_folders_send(xml_file_path, user_name, key_path,
                 base_path, nodes_config)
  f_config_send(xml_file_path, user_name, key_path,
  ...
  f_compile(xml_file_path, user_name, key_path, nodes_config)
  f_sw_config_gene(xml_file_path, nodes_config['load_balancer'])
  print('Have printed the switch configurations,'
        'please input all of them into the consoles of switch.')
  

def f_latency_intra_config(xml_file_path, user_name, key_path,
                           base_path):
  config_file_path = os.path.join(base_path, 'config.json')
  active_nodes = f_active_nodes_get(xml_file_path, user_name, key_path)
...
def f_latency_inter_config(xml_file_path, user_name, key_path,
                     base_path):
  config_file_path = os.path.join(base_path, 'config.json')
  active_nodes = f_active_nodes_get(xml_file_path, user_name, key_path)
  nodes_config = f_latency_intra_config_gene(config_file_path, active_nodes)
...
def f_rate_clear(xml_file_path, user_name, key_path,
                 config_file_path):
  try:
  ...
  print('Have printed the configurations to clear switch,'
        'please input all of them into the consoles of switch.')
  

def f_load_clear(xml_file_path, user_name, key_path,
                 config_file_path):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
...
def f_rate_run(xml_file_path, user_name, key_path, 
               config_file_path, if_parallel, frequency):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  except FileNotFoundError:
  ...
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  f_rate_services_remove(xml_file_path, user_name, key_path, 
                         nodes_config)
  ...
  time.sleep(5)
  f_rate_ss_deploy(xml_file_path, user_name, key_path, 
                   nodes_config, if_parallel, frequency)
  ...
  f_rate_ss_result_pull(xml_file_path, user_name, key_path, 
                        nodes_config, local_file_path)
  f_rate_services_remove(xml_file_path, user_name, key_path, 
                         nodes_config)
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  ...
  print("")
  print(f'Have finished the experiment, the results for Fig.10 have been outputed \n'
        f'the results for Fig.10 are also in file results/freq/{file_name}.')
  

def f_bound_run(xml_file_path, user_name, key_path, 
                config_file_path):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  ...
  f_rate_requests_deploy(xml_file_path, user_name, key_path, requests)
  time.sleep(5)
  f_bound_ss_deploy(xml_file_path, user_name, key_path, 
                   nodes_config)
  time_name = datetime.datetime.now()
  timestamp = time_name.strftime("%Y%m%d%H%M%S")
  real_scale = scale * 64;
  ...
  f_bound_ss_result_pull(xml_file_path, user_name, key_path, 
                        nodes_config, local_file_path)
  f_rate_services_remove(xml_file_path, user_name, key_path, 
                         nodes_config)
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  print(f'Have finished the experiment.\n'
...
def f_accuracy_run(xml_file_path, user_name, key_path, 
                   config_file_path, frequency):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  except FileNotFoundError:
  ...
  f_rate_requests_deploy(xml_file_path, user_name, key_path, requests)
  time.sleep(5)
  f_accuracy_ss_deploy(xml_file_path, user_name, key_path, 
                       nodes_config, frequency)
  time_name = datetime.datetime.now()
  timestamp = time_name.strftime("%Y%m%d%H%M%S")
  real_scale = scale * 64;
  file_name = f'freq_{frequency}_{real_scale}_{timestamp}.txt'
  local_file_path = os.path.join('results/accuracy/', file_name)
  f_accuracy_ss_result_pull(xml_file_path, user_name, key_path, 
                            nodes_config, local_file_path)
  f_rate_services_remove(xml_file_path, user_name, key_path, 
                         nodes_config)
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
  ...
  f_print_file_contents(local_file_path)
  print("")
  print(f'Have finished the experiment, the results for Fig.11 have been outputed \n'
        f'the results for Fig.11 are also in file results/accuracy/{file_name}.')


def f_load_run(xml_file_path, user_name, key_path, 
               load_type, if_snapshot, config_file_path):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  except FileNotFoundError:
  ...
  if (real_type == 'iperf'):
    ...
    f_load_iperf_requests_deploy(xml_file_path, user_name, key_path, 
                                 requests, details)
    time.sleep(45)
    real_scale = scale * 64
    time_name = datetime.datetime.now()
    ...
    f_load_iperf_result_pull(xml_file_path, user_name, key_path, 
                             nodes_config, local_file_prefix)
    f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
    f_rate_services_remove(xml_file_path, user_name, key_path,
                           nodes_config)
    ...
    f_rate_progs_remove(xml_file_path, user_name, key_path, 
                        nodes_config)
    f_load_cassandra_remove(xml_file_path, user_name, key_path, 
                            nodes_config)
    f_rate_progs_run(xml_file_path, user_name, key_path, nodes_config)
    ...
    if (if_snapshot):
      f_load_ss_deploy(xml_file_path, user_name, key_path, nodes_config)
    f_load_cassandra_request(xml_file_path, user_name, key_path, 
                             requests, details)
    time_name = datetime.datetime.now()
    timestamp = time_name.strftime("%Y%m%d%H%M%S")
    real_scale = scale * 64
    ...
    f_load_cassandra_result_pull(xml_file_path, user_name, key_path, 
                                 nodes_config, local_file_path)
    f_load_cassandra_remove(xml_file_path, user_name, key_path, 
                            nodes_config)
    f_rate_progs_remove(xml_file_path, user_name, key_path,
                        nodes_config)
  print('Have finished the experiment, check folder '
        'results/load/ for results!')
  

def f_latency_intra_run(xml_file_path, user_name, key_path,
                        config_file_path):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  ...
  nodes_config = config_data['nodes']
  print("Start to run the experiment!")
  f_latency_intra_progs_remove(xml_file_path, user_name, key_path, 
                               nodes_config)
  ...
  f_latency_intra_result_pull(xml_file_path, user_name, key_path, 
                              nodes_config, local_file_path)
  f_latency_intra_progs_remove(xml_file_path, user_name, key_path, 
                               nodes_config)
  print(f'Have finished the experiment.\n'
...
def f_latency_internet_run(xml_file_path, user_name, key_path,
                           config_file_path):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  ...
  f_latency_intra_progs_remove(xml_file_path, user_name, key_path, 
                               nodes_config)
  f_latency_internet_progs_run(xml_file_path, user_name, key_path, 
                               nodes_config)
...
def f_latency_inter_run(xml_file_path, user_name, key_path,
                        config_file_path):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
...
def f_bot_beaver_run(xml_file_path, user_name, key_path,
                     config_file_path, ratio):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  except FileNotFoundError:
  ...
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  f_bot_programs_run(xml_file_path, user_name, key_path, 
                     nodes_config)
  time.sleep(5)
  ...
  f_bot_result_pull(xml_file_path, user_name, key_path, 
                    nodes_config, local_file_path)
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  

def f_bot_poll_run(xml_file_path, user_name, key_path,
                   config_file_path, ratio):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  ...
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  f_bot_poll_programs_run(xml_file_path, user_name, key_path, 
                          nodes_config)
  client_index = nodes_config['client'][0]
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  client_ip = ssh_ips['node' + str(client_index)]
...
def f_bot_laiyang_run(xml_file_path, user_name, key_path,
                   config_file_path, ratio):
  try:
    with open(config_file_path, 'r') as file:
      config_data = json.load(file)
  ...
  f_rate_progs_remove(xml_file_path, user_name, key_path, 
                      nodes_config)
  f_bot_laiyang_programs_run(xml_file_path, user_name, key_path, 
                             nodes_config)
  client_index = nodes_config['client'][0]
  ssh_ips = f_map_ssh_ip_get(xml_file_path)
  client_ip = ssh_ips['node' + str(client_index)]
