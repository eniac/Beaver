...
def f_args_parse():
  ...
  parser_ss_rate.add_argument(
    '-o', '--operation', type=str, 
    choices=['config', 'run', 'clear'], required=True,
  ...
  parser_ss_accuracy.add_argument(
      ...
      help='Scale of the load balancers, must be specified.')
  parser_ss_accuracy.add_argument(
      '-f', '--frequency', type=int, required=True, 
      help='Frequency of snapshot.')
  parser_ss_accuracy.add_argument(
    '-o', '--operation', type=str, 
    choices=['config', 'run', 'clear'], required=True,
    help='Specify the operation of the experiment:' 
  ...
  parser_replace.add_argument(
      '-nt', '--node_type', type=str, required=True, 
  ...
  parser_load.add_argument(
      ...
      help='Scale of the load balancers, must be specified.')
  parser_load.add_argument(
      '-lt', '--load_type', type=str, required=True,
      help='Type of workload.')
  parser_load.add_argument(
      '-ss', '--snapshot', action='store_true',
      help='Whether to do snapshots.')
  parser_load.add_argument(
    '-o', '--operation', type=str, 
    choices=['config', 'run', 'clear'], required=True,
    help='Specify the operation of the experiment:' 
  ...
  parser_latency.add_argument(
    ...
    'clear for removing all configuration.')
  parser_latency.add_argument(
    '-lt', '--latency_type', type=str, required=True, 
  ...
  parser_bot.add_argument(
      ...
      help='Ratio of bot.')
  parser_bot.add_argument(
      '-st', '--snapshot_type', type=str, 
      choices=['beaver', 'laiyang', 'poll'],required=True, 
...
def main():
  args = f_args_parse()
  if (args.type == 'env'):
    f_set_env(args.manifest_path, args.user_name,
              args.ssh_key, 'install_env.sh')
  elif (args.type == 'replace'):
    f_replace_folders(args.manifest_path, args.user_name, args.ssh_key,
                      './config.json', args.node_type)
  elif (args.type == 'rate'):
    cluster_scale = args.scale // 64
    frequency_list1 = [300000, 240000, 200000, 160000, 140000, 130000, 100000, 75000]
    ...
    if (args.operation == 'config'):
      f_rate_config(args.manifest_path, args.user_name, args.ssh_key,
                    cluster_scale, './')
    if (args.operation == 'run'):
      f_rate_run(args.manifest_path, args.user_name,
                 args.ssh_key, 'config.json', 
                 args.parallel, frequency)
    if (args.operation == 'clear'):
      f_rate_clear(args.manifest_path, args.user_name, args.ssh_key,
                   'config.json')
    ...
    if (args.operation == 'config'):
      ...
      f_bound_run(args.manifest_path, args.user_name,
                  args.ssh_key, 'config.json')
    ...
    if (args.operation == 'config'):
      f_rate_config(args.manifest_path, args.user_name, args.ssh_key,
                    cluster_scale, './')
    if (args.operation == 'run'):
      f_accuracy_run(args.manifest_path, args.user_name,
                     args.ssh_key, 'config.json', args.frequency)
    if (args.operation == 'clear'):
      f_rate_clear(args.manifest_path, args.user_name, args.ssh_key,
                   'config.json')
  elif (args.type == 'load'):
    ...
    if (args.operation == 'config'):
      f_load_config(args.manifest_path, args.user_name, args.ssh_key,
                    cluster_scale, args.load_type, './')
    if (args.operation == 'run'):
      f_load_run(args.manifest_path, args.user_name,
                 args.ssh_key, args.load_type, 
                 args.snapshot, 'config.json')
    if (args.operation == 'clear'):
      f_load_clear(args.manifest_path, args.user_name, args.ssh_key,
                   'config.json')
  elif (args.type == 'latency'):
    if (args.operation == 'config'):
      f_latency_intra_config(args.manifest_path, args.user_name, 
                             args.ssh_key, './')
    elif (args.operation == 'run'):
      if (args.latency_type == 'intra'):
        f_latency_intra_run(args.manifest_path, args.user_name,
                            args.ssh_key, 'config.json')
      elif (args.latency_type == 'internet'):
        f_latency_internet_run(args.manifest_path, args.user_name,
                               args.ssh_key, 'config.json')
      elif (args.latency_type == 'inter'):
        f_latency_inter_run(args.manifest_path, args.user_name,
                            args.ssh_key, 'config.json')
    elif (args.operation == 'clear'):
      f_load_clear(args.manifest_path, args.user_name, args.ssh_key,
                   'config.json')
    ...
    if (args.operation == 'config'):
      ...
      if (args.snapshot_type == 'beaver'):
        f_bot_beaver_run(args.manifest_path, args.user_name, args.ssh_key,
                         'config.json', args.ratio)
      elif (args.snapshot_type == 'poll'):
        f_bot_poll_run(args.manifest_path, args.user_name, args.ssh_key,
                       'config.json', args.ratio)
      elif (args.snapshot_type == 'laiyang'):
        f_bot_laiyang_run(args.manifest_path, args.user_name, args.ssh_key,
                          'config.json', args.ratio)
    elif (args.operation == 'clear'):
      f_load_clear(args.manifest_path, args.user_name, args.ssh_key,
                  'config.json')
