...
def f_nodes_config_gene(lb_num, active_nodes):
  ...
  if (nodes_num < required_nodes_num):
    print("The number of booked nodes is not enough,"
          "please reduce the scale.")
    exit(1)
  controller_index = [1]
...
def f_sw_config_gene(xml_file_path, lbs_index):
  sw_config_cmds = []
  ctrl_plane_ips, data_plane_ips, sw_gateway_ips = \
      f_maps_local_ip_get(xml_file_path)
  ...
  for index in lbs_index:
    ...
    sw_config_cmds.append(f"ip route 192.168.100.0 255.255.255.0 {lb_ip_data_plane}")
...
def f_sw_clear_gene(xml_file_path, lbs_index):
  ...
  for index in lbs_index:
    lb_name = f"node{index}"
    lb_ip_data_plane = data_plane_ips[lb_name]
    sw_config_cmds.append(f"no ip route 192.168.100.0 255.255.255.0 {lb_ip_data_plane}")
  print("Switch configuration:\n")
  for command in sw_config_cmds:
    print(command)
  print("\n")
...
def f_rate_services_config_gene(backends_index, clients_index):
  ...
  for backend_index in backends_index:
    ...
    protocol = "udp"
    # protocol = "tcp"
    service = f_rate_service_config_gene(backend_ip, backend_port, 
                                         virt_ip, virt_port, protocol, 
                                         node_name)
  ...
  for i in range(len(backends_index)):
    ...
    virt_port = str(10000)
    protocol = "udp"
    request = f_rate_request_config_gene(backend_ip, virt_ip,
                                         virt_port, protocol, node_name)
  ...
  for i in range(client_request_num):
    ...
    virt_port = str(10000)
    protocol = "udp"
    request = f_rate_request_config_gene(client_ip, virt_ip,
                                         virt_port, protocol, node_name)
    requests.append(request)
...
def f_load_services_config_gene(backends_index, clients_index, load_type):
  ...
  if (real_type == 'iperf'):
    ...
    for backend_index in backends_index:
      ...
      virt_port = str(10000)
      service = f_load_cassandra_config_gene(backend_ip, backend_port, 
                                             virt_ip, virt_port, 
...
def f_latency_intra_services_gene(backends_index):
  ...
  for backend_index in backends_index:
    ...
    virt_port = str(10000)
    protocol = "udp"
    service = f_latency_service_config_gene(backend_ip, backend_port, 
                                           virt_ip, virt_port, protocol, 
                                           node_name)
    services.append(service)
    break
...
def f_bot_services_gene(backends_index):
  ...
  for backend_index in backends_index:
    ...
    virt_port = str(10000)
    protocol = "udp"
    service = f_bot_service_config_gene(backend_ip, backend_port, 
                                        virt_ip, virt_port, protocol, 
                                        node_name)
    services.append(service)
    index += 1
