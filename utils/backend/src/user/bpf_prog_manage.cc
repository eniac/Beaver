...
int f_bpf_ingress_prog_load(const char *prog_pathname, int if_index) {
  ...
  struct bpf_map *snat_info = bpf_object__find_map_by_name(
                                  prog_obj, "snat_info");
  if (snat_info == NULL) {
  ...
  struct bpf_map *ss_info = bpf_object__find_map_by_name(
                                prog_obj, "ss_info");
  if (ss_info == NULL) {
  ...
  struct bpf_map *bot_record = bpf_object__find_map_by_name(
                                   prog_obj, "bot_record");
  if(bot_record == NULL) {
...
int f_bpf_egress_prog_load(const char *prog_pathname, int if_index) {
  ...
  struct bpf_map *virt_info_reqs = bpf_object__find_map_by_name(
                                       prog_obj, "virt_info_reqs");
  if (virt_info_reqs == NULL) {
    printf("Error finding map virt_info_reqs.\n");
    return -1;
  }
  struct bpf_map *existing_reqs = bpf_object__find_map_by_name(
                                      prog_obj, "existing_reqs");
  ...
  DECLARE_LIBBPF_OPTS(
      bpf_tc_hook, tc_hook,
	    .ifindex = if_index, .attach_point = BPF_TC_EGRESS);
  DECLARE_LIBBPF_OPTS(
      bpf_tc_opts, tc_opts,
...
int f_bpf_prog_unload(int if_index) {
  ...
  DECLARE_LIBBPF_OPTS(
      bpf_tc_hook, tc_hook, .ifindex = if_index, 
      .attach_point = static_cast<bpf_tc_attach_point> 
                      (BPF_TC_EGRESS | BPF_TC_INGRESS));
...
int f_nic_ifindex_get_data_plane(const char *ip_prefix) {
  ...
  for (uint32_t i = 0; i < num_interfaces; i++) {
    ...
    if (strstr(interface_ip, ip_prefix) == interface_ip &&
        !strstr(interface_ip, "192.168.255")) {
