...
void f_service_bot_test() {
  ...
  if (bind(sock_fd, (const struct sockaddr *)&service_addr, 
  ...
  while (true) {
    int message_len = recvfrom(sock_fd, buffer, 4096, MSG_WAITALL, 
    ...
    if (strcmp(buffer, "Request") == 0) {
      sendto(sock_fd, (const char *)buffer, strlen(buffer), MSG_CONFIRM,
      ...
      for (auto &bot_data : bot_record_vec) {
        sendto(sock_fd, (const char *)&bot_data, sizeof(bot_data), MSG_CONFIRM,
               (const struct sockaddr *)&client_addr, client_len);
      }
      bot_record_vec.clear();
      const char* end_message = "end";
      sendto(sock_fd, end_message, strlen(end_message), MSG_CONFIRM,
             (const struct sockaddr *)&client_addr, client_len);
    }
...
void f_ly_service_logic() {
  ...
  if ((bind(sock_fd, (struct sockaddr*)& service_addr, 
            sizeof(service_addr))) != 0) {
...
void f_ly_client_logic(char *ip) {
  ...
  if (bind(sock_fd, (struct sockaddr*)& client_addr, 
  ...
  while (attempt_count < 5) {
    connect_result = connect(sock_fd, (struct sockaddr*)&service_addr, 
                             sizeof(service_addr));
    if (connect_result == 0) {
