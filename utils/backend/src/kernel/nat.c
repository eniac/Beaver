...
*/
int f_ssid_get_tcp(struct xdp_md *ctx, struct ethhdr *eth_hdr, struct iphdr *ip_hdr) {
  struct tcphdr* tcp_hdr = (struct tcphdr *)(ip_hdr + 1);
  if ((void *)(tcp_hdr + 1) > (void *)(long)ctx->data_end)
    return XDP_PASS;

  if (tcp_hdr->dest == bpf_htons(kIntraServicePort) ||
      tcp_hdr->source == bpf_htons(kIntraServicePort)) {
...
*/
int f_nat_udp(struct xdp_md *ctx, struct ethhdr *eth_hdr, struct iphdr *ip_hdr) {
  ...
  struct PackedPortProto snat_key = {.port = udp_hdr->dest,
                                     .protocol = inner_ip_hdr->protocol};
  if (inner_ip_hdr->protocol == kProtoUdp) {
    ...
    new_checksum = f_csum_update_for_port(inner_udp_hdr->dest, udp_hdr->dest, 
                                          new_checksum);
    ...
    new_checksum = f_csum_update_for_port(inner_tcp_hdr->dest, udp_hdr->dest, 
                                          new_checksum);
  ...
  struct PackedVirtInfo *snat_virt_info = bpf_map_lookup_elem(&snat_info, 
                                                              &snat_key);
  if (!snat_virt_info) {
