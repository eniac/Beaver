...
*/
int f_ssid_load(struct iphdr *ip_hdr) {
  __u32 ssid_key = 1;
...
*/
int f_virt_info_load(struct iphdr *ip_hdr, struct udphdr *udp_hdr) {
  struct PackedPortProto port_proto = {.port = udp_hdr->source,
                                       .protocol = ip_hdr->protocol};
  struct PackedVirtInfo *virt_info_ptr = bpf_map_lookup_elem(&snat_info, 
                                                             &port_proto);
  if (!virt_info_ptr) {
    struct PackedVirtInfoReq virt_info_req = {.ip = ip_hdr->saddr, 
                                              .port = udp_hdr->source,
                                              .protocol = ip_hdr->protocol,
    ...
    if(!existing) {
      __u32 status = 1;
      bpf_map_update_elem(&existing_reqs, &virt_info_req, 
                          &status, BPF_NOEXIST);
...
int f_snat(struct __sk_buff *skb) {
  ...
  if (eth_hdr.h_proto == bpf_htons(ETH_P_IP)) {
    ...
    if (ip_hdr.protocol == kProtoUdp || ip_hdr.protocol == kProtoTcp) {
      ...
      if (udp_hdr.dest == bpf_htons(kIntraServicePort) || 
          udp_hdr.source == bpf_htons(kIntraServicePort)) {
        if (f_ssid_load(&ip_hdr) < 0)
          return BPF_DROP;
        bpf_skb_store_bytes(skb, sizeof(eth_hdr), 
                            &ip_hdr, sizeof(ip_hdr), 0);
        return BPF_OK;
      }
      ...
      bpf_skb_store_bytes(skb, sizeof(eth_hdr), 
                          &ip_hdr, sizeof(ip_hdr), 0);
      bpf_skb_store_bytes(skb, sizeof(eth_hdr) + sizeof(ip_hdr), 
                          &udp_hdr, sizeof(udp_hdr), 0);
