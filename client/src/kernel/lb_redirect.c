...
int f_lb_redirect(struct __sk_buff *skb) {
  ...
  if (eth_hdr.h_proto == bpf_htons(ETH_P_IP)) {
    ...
    if (ip_hdr.protocol == IPPROTO_UDP || ip_hdr.protocol == IPPROTO_TCP) {
      ...
      if (bpf_ntohs(udp_hdr.dest) >= kDstPortRedirect) {
        ...
        __u32 port_hash_key = f_port_hash_key_get(udp_hdr.source,
                                                  udp_hdr.dest);
        bpf_map_update_elem(&virt_ip_hash_table, &port_hash_key, 
                            &virt_ip, BPF_ANY);
        return BPF_OK;
      }
    }
