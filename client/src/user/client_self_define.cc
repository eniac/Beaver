...
void f_latency_test_client(const std::string& service_ip, int service_port) {
  ...
  if (bind(sock_fd, (const struct sockaddr *)&client_addr, 
  ...
  while (request_counter < 100) {
    for (int i = 0; i < 2; i++) {
      sendto(sock_fd, message.c_str(), message.length(), MSG_CONFIRM, 
             (const struct sockaddr *)&service_addr, sizeof(service_addr));
      recvfrom(sock_fd, buffer, sizeof(buffer), MSG_WAITALL, 
               (struct sockaddr *)&service_addr, &len);
    }
    request_counter++;
...
void f_udp_message_send(const char *message) {
  ...
  n = recvfrom(sock_fd, buffer, 1024, 0, 
               (struct sockaddr *)&service_addr, &len);
  if (n > 0) {
      buffer[n] = '\0';
  } else {
...
void f_bot_udp_req(int sock_fd, struct sockaddr_in service_addr, 
  ...
  if (sendto(sock_fd, message, strlen(message), 0, 
             (struct sockaddr *)&service_addr, sizeof(service_addr)) < 0) {
  ...
  if (recvfrom(sock_fd, buffer, 1024, 0, 
               (struct sockaddr *)&service_addr, &service_addr_len) < 0) {
...
void f_bot_results_collect(int sock_fd, struct sockaddr_in service_addr, 
  ...
  while (true) {
    ssize_t n = recvfrom(sock_fd, buffer, 1024, 0, 
                         (struct sockaddr *)& service_addr, &service_addr_len);
    if (n < 0) {
      perror("Recvfrom failed");
      exit(EXIT_FAILURE);
    }
...
void f_bot_beaver_test(float bot_ratio) {
  ...
  output_file << "True Positive: " 
              << (float) true_positive / total_requests << std::endl;
  output_file << "False Positive: " 
              << (float) false_positive / total_requests << std::endl;
  output_file << "True Negative: " 
              << (float) true_negative / total_requests << std::endl;
  output_file << "False Negative: " 
              << (float) false_negative / total_requests << std::endl;
  output_file.close();
}
...
void f_bot_poll_test(float bot_ratio) {
  ...
  char message[] = "start";
  if (sendto(sock_fd, message, strlen(message), 0, 
             (struct sockaddr *)&service_addr1, sizeof(service_addr1)) < 0) {
    perror("Sendto failed");
    exit(EXIT_FAILURE);
  }
  if (sendto(sock_fd, message, strlen(message), 0, 
             (struct sockaddr *)&service_addr2, sizeof(service_addr2)) < 0) {
    perror("Sendto failed");
    exit(EXIT_FAILURE);
  }
  usleep(10000);
