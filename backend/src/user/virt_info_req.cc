...
f_virt_info_req(__u16 port)
{
    ...
    while (1) {
        ...
        ret = bpf_map_lookup_and_delete_elem(virt_info_reqs_fd, NULL, &virt_info_req);
        if (ret != 0)
            continue;
        struct PackedVirtInfo virt_info;
        ret = sendto(sock, &virt_info_req, sizeof(virt_info_req), 0, (struct sockaddr*)&manager_addr, addr_size);
        if (ret < 0) {
            printf("Error sending request.\n");
            close(sock);
            break;
        }
        int recv_size = recvfrom(sock, &virt_info, sizeof(virt_info), 0, (struct sockaddr*)&manager_addr, &addr_size);
        if (recv_size > 0) {
            struct PackedPortProto port_proto;
            port_proto.port = virt_info_req.port;
            port_proto.protocol = virt_info_req.protocol;
            ret = bpf_map_update_elem(snat_info_fd, &port_proto, &virt_info, BPF_ANY);
            if (ret < 0) {
